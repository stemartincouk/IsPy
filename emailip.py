...
class Conf:
    def __init__(self):
        self.fileLocation="/opt/emailip/emailip.conf"
        self.mailServer=""
        self.mailServerAuth=""
        self.mailUser=""
        self.mailPass=""
...
class Test:
    ...
    def self_test(self):
        ...
        if cfig.check() ==0:
            ...
            self.fault="There was a problem with your config file please run emailip.oy -s"
            self.result=0
        if self.mail_test()==0:
...
def logger(type,text):
    ...
    logfile =open("/opt/emailip/emailip.log","a")
